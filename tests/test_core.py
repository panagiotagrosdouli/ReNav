...
def trap():
    ...
    model = ActionTriggeredHazardModel(
        (ActionTriggeredClosure(((2, 1), (3, 1)), (1, 1), 0.8),)
    )
...
def test_same_state_different_history():
    ...
    assert history_conditioned_return_probability(
        grid, goal, {start}, model, frozenset()
    ) == pytest.approx(1.0)
    assert history_conditioned_return_probability(
        grid, goal, {start}, model, frozenset({0})
    ) == pytest.approx(0.2)
...
def test_parallel_closures_match_analytic_probability():
    grid = GridMap.from_obstacles(3, 3, {(1, 1)})
    probability = exact_safe_return_probability(
        grid,
        (2, 1),
        {(0, 1)},
        TopologyHazardBelief({(1, 0): 0.2, (1, 2): 0.3}),
    )
    assert probability == pytest.approx(1.0 - 0.2 * 0.3)


def test_invalid_active_index_is_rejected():
    grid, start, goal, model = trap()
    with pytest.raises(ValueError, match="out of range"):
        history_conditioned_return_probability(
            grid, goal, {start}, model, frozenset({4})
        )
